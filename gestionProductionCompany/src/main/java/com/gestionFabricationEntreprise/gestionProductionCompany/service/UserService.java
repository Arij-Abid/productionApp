package com.gestionFabricationEntreprise.gestionProductionCompany.service;

import java.io.IOException;
import java.util.List;

import javax.mail.MessagingException;

import org.springframework.web.multipart.MultipartFile;

import com.gestionFabricationEntreprise.gestionProductionCompany.domain.User;
import com.gestionFabricationEntreprise.gestionProductionCompany.entities.EtapeProduction;
import com.gestionFabricationEntreprise.gestionProductionCompany.exception.domain.EmailExistException;
import com.gestionFabricationEntreprise.gestionProductionCompany.exception.domain.EmailNotFoundException;
import com.gestionFabricationEntreprise.gestionProductionCompany.exception.domain.NotAnImageFileException;
import com.gestionFabricationEntreprise.gestionProductionCompany.exception.domain.UserNotFoundException;
import com.gestionFabricationEntreprise.gestionProductionCompany.exception.domain.UsernameExistException;

public interface UserService {

    User register(String firstName, String lastName, String username, String email) throws UserNotFoundException, UsernameExistException, EmailExistException, MessagingException;

    List<User> getUsers();

    User findUserByUsername(String username);

    User findUserByEmail(String email);

    User addNewUser(String firstName, String lastName, String username, String email, String role, boolean isNonLocked, boolean isActive, MultipartFile profileImage) throws UserNotFoundException, UsernameExistException, EmailExistException, IOException, NotAnImageFileException;

    User updateUser(String currentUsername, String newFirstName, String newLastName, String newUsername, String newEmail, String role, boolean isNonLocked, boolean isActive, MultipartFile profileImage) throws UserNotFoundException, UsernameExistException, EmailExistException, IOException, NotAnImageFileException;

    void deleteUser(String username) throws IOException;

    void resetPassword(String email) throws MessagingException, EmailNotFoundException;

    User updateProfileImage(String username, MultipartFile profileImage) throws UserNotFoundException, UsernameExistException, EmailExistException, IOException, NotAnImageFileException;
    List<User> getUsersByRole(String role);

   // User assignEtapeProduction(User employee, EtapeProduction etapeProduction);
    
    User getUserById(Long userId);
   List<User> getAllEmployees();
	User getEmployeesById(long id);
    User getEmployeeByIdAndRole(Long id, String role);
     User getEmployeesById(String userId);
}
